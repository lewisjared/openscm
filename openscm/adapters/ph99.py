...
class PH99(Adapter):
    ...
    _hc_per_m2_approx = 30 ** 6 * unit_registry("J / kelvin / m^2")
    _ecs = None
    ...
    def _initialize_model_input(self) -> None:
        ...
        # TODO: add convenience function to get start of years in seconds for
        # passing inputs up/down
        ...
        emms_view = self._parameters.get_timeseries_view(
            ("Emissions", "CO2"),
            ("World",),
            str(self.model.emissions.units),
            time_points,
            ParameterType.AVERAGE_TIMESERIES,
        ...
        if emms_view.is_empty:
            ...
            self.model.emissions = emms_view.get() * self.model.emissions.units
    ...
    def _set_model_parameter(self, para_name, value):
        try:
            ...
            raise NotAnScmParameterError(
                "{} is not a {} parameter".format(para_name, self.__class__.__name__)
            )
